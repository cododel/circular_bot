...
У обычного видео берётся центральный квадрат кадра — дальше пайплайн тот же,
что и для кружков. Размер входящего файла ограничен лимитом Bot API (20 МБ,
настраивается через `MAX_VIDEO_SIZE_MB`).
